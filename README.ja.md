...
**アプリだけ欲しい方へ。** 最新ビルドをどうぞ：

### → [**Macsist.dmg をダウンロード**](https://github.com/junidude/macsist/releases/latest/download/Macsist.dmg)

このビルドは**自己署名**（Apple の公証なし）のため、初回起動時に macOS が *「Apple は『Macsist』にマルウェアが含まれていないことを確認できません」* と表示してブロックします —— 想定どおりで、マルウェアでは**ありません**。**「ゴミ箱に入れる」を押さないでください：**

1. **Macsist** を**アプリケーション**にドラッグし、警告ダイアログで**「完了」**を押します。
2. **システム設定 → プライバシーとセキュリティ**を開き、下にスクロールして Macsist のメッセージの横の**「このまま開く」**を押し、確認すると開きます。
   *(ターミナルでの代替：`xattr -dr com.apple.quarantine /Applications/Macsist.app`)*

以降は通常どおりダブルクリックで開きます。初回起動時に外部 API かローカルモデルかを尋ねられます。
